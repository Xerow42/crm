...
def admin_dashboard():
    ...
    cursor.execute("""
        SELECT r.id, u.nom, m.nom_motif, r.description, r.description_originale,
            r.langue_origine, r.sentiment, r.date_reclamation
        FROM reclamations r 
        JOIN utilisateurs u ON r.id_utilisateur = u.id
        JOIN motifs_email m ON r.id_motif = m.id
        ORDER BY r.date_reclamation ASC
    """)
    reclamations = [dict(rec) for rec in cursor.fetchall()]


    # Traduire le motif si besoin
    for rec in reclamations:
        rec['nom_motif'] = GoogleTranslator(source='auto', target='fr').translate(rec['nom_motif'])
...
def admin_dashboard_dev():
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT r.id, u.nom, m.nom_motif, r.description, r.description_originale, r.langue_origine, r.sentiment, r.compound_score, r.date_reclamation
        FROM reclamations r 
        JOIN utilisateurs u ON r.id_utilisateur = u.id
        JOIN motifs_email m ON r.id_motif = m.id
        ORDER BY r.date_reclamation ASC
...
def importer_csv(fichier_csv):
    ...
    with open(fichier_csv, newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            nom = row.get('nom')
            email = row.get('email')
            motif_nom = row.get('motif', 'Autre')
            description_originale = row.get('description')
            date_reclamation = row.get('date_reclamation', datetime.now().strftime('%Y-%m-%d'))
            cursor.execute("SELECT id FROM motifs_email WHERE nom_motif = ?", (motif_nom,))
            motif = cursor.fetchone()
            if motif:
                id_motif = motif[0]
            else:
                cursor.execute("INSERT INTO motifs_email (nom_motif) VALUES (?)", (motif_nom,))
                id_motif = cursor.lastrowid
            ...
            cursor.execute('''
                ...
                (id_utilisateur, id_motif, description, date_reclamation, langue_origine, description_originale, sentiment, compound_score) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
